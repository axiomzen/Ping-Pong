        ...
        bookshelf.knex
          ...
          .then(function(games) {
            games.forEach(function(game) {
              var comparator = criteria === 'wins' ? game.winner_id ===
                player.id : game.winner_id !== player.id;
              if (comparator) {
...
function largestWhooping() {
  ...
  bookshelf.knex
    ...
    .then(function(game) {
      ...
      async.parallel([
        function(cb) {
          bookshelf.knex.select('name').from('players').where('id', game.player0_id)
            .then(function(player) {
        ...
        function(cb) {
          bookshelf.knex.select('name').from('players').where('id', game.player1_id)
            .then(function(player) {
