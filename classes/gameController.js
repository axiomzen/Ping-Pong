...
function gameController() {
  ...
  elo.on('tip.playerWin', function(player) {
    ...
    io.sockets.emit('game.message', {
      message: 'A win for <span class="player-' + player.position + '">' +
        player.name + '</span> takes ' + pronoun + ' to rank ' +
        player.winningLeaderboardRank
    });
...
gameController.prototype.end = function(complete) {
  ...
  io.sockets.emit('game.message', {
    message: '<span class="player-0">' + players[0].get('name') +
      '</span> is now rank ' + updatedRanks[0] + ', <span class="player-1">' +
      players[1].get('name') + '</span> is rank ' + updatedRanks[1]
  });
  ...
  console.log(chalk.green('Game ending, ' + players[winningPlayer - 1].get('name') +
    ' won'));
...
gameController.prototype.start = function(startingServe) {
  ...
  if (!game.minPlayersAdded()) {
    console.log(chalk.red('Can\'t start the game until ' + settings.minPlayers +
      ' players have joined'));
    return false;
  }
...
gameController.prototype.checkWon = function() {
  ...
  var
    playerReachedMaxScore = game.score[0] >= settings.maxScore || game.score[1] >=
    settings.maxScore,
...
gameController.prototype.checkServerSwitch = function(forceServe) {
  ...
  var
    ...
    pointJustCancelled = this.gameHistory.length > 0 && this.gameHistory[0].action ==
    'cancelPoint',
    switchServer = totalScore % settings.serverSwitchLimit === 0 || this.serverSwitchThresholdMet() ||
    typeof forceServe !== 'undefined',
    switchPreviousServer = (totalScore + 1) % settings.serverSwitchLimit === 0 &&
    pointJustCancelled;
...
gameController.prototype.nextPointWins = function() {
  ...
  var
    ...
    futureScoreDifference = (nextScorePlayer1 > nextScorePlayer2) ? nextScorePlayer1 -
    nextScorePlayer2 : nextScorePlayer2 - nextScorePlayer1;

  return (nextScorePlayer1 >= settings.maxScore || nextScorePlayer2 >= settings.maxScore) &&
    (futureScoreDifference + 1 >= settings.mustWinBy);
